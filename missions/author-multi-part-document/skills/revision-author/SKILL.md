...
`sotFollowUpCount`, `revisionRetentionMode`, `priorRevisionToken`,
`workingRevisionToken`, `revisionArchivePath`, `revisionReverted`,
`backupsRemoved`, `continuationStatus`
...
| R3 | Populate **`outputs`** from the required field list above; `reviewComplete` only after user confirmation and SoT conversation review completes per step 8; set `markupMode` / `markupPending` per § *Pending markup mode*; set revision-retention fields after § *Post-content-approval revision retention* |
